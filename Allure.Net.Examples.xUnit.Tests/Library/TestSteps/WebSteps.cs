...
namespace Allure.Net.Examples.xUnit.Tests.Library.TestSteps
{
    public static class WebSteps
    {
        ...
        {
            if (IsTimeToThrowException()) Assert.True(false, WebDriverIsNotReachableMessage("Allure"));
        }

        private static void MaybeThrowElementNotFoundException()
        {
            Thread.Sleep(1000);
            if (IsTimeToThrowException()) Assert.True(false, ElementNotFoundMessage("[//div[@class='something']]"));
        }
